...
- `POST /v1/messages` - Anthropic-compatible messages endpoint (honors `system` and `tools`; supports streaming via `"stream": true` with native Anthropic SSE events; tool calls are emulated via prompt-based function calling and returned as `tool_use` blocks)
